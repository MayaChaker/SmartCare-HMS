...
export const ReceptionistProvider = ({ children }) => {
  ...
  // Common reasons presented when scheduling
    ...
    try {
      ...
      const payload = {
        ...
        appointmentTime: appointmentForm.appointmentTime
          ? `${toHHMM(appointmentForm.appointmentTime)}:00`
          : undefined,
      };
  ...
  // Mark an appointment as checked-in and reload dashboard data.
  ...
  useEffect(() => {
    const fetchEditTimes = async () => {
      ...
      try {
        ...
        // Parse working hours from the doctor; fallback to appointment's doctor
        const doc = doctors.find((d) => String(d.id) === String(doctorId));
        const parsed = parseWorkingHours(
  ...
  // register a new patient with core
  const renderRegisterPatientModal = () => (
  ...
  const renderScheduleAppointmentModal = () => (
            ...
            `/api/receptionist/appointments/${selectedItem.id}`,
            {
              ...
              body: JSON.stringify({
                ...
                ...(editForm.appointmentTime
                  ? {
                      appointmentTime: `${toHHMM(editForm.appointmentTime)}:00`,
                    }
                  : {}),
              }),
            }
    ...
    >
      <div className="form-grid">
        ...
        <div className="form-group">
          ...
          <select
            value={
              editForm.appointmentTime || selectedItem?.appointmentTime || ""
            }
            onChange={(e) =>
              setEditForm({
                ...editForm,
        ...
        <div className="form-group">
          <label>Status</label>
          <select
            value={editForm.status || selectedItem?.status || "scheduled"}
            onChange={(e) =>
      ...
      <div className="modal-overlay" onClick={closeModal}>
        <div className="modal" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="modal-content">
            ...
            {/* ===== register patient ===== */}
            {modalType === "registerPatient" && renderRegisterPatientModal()}

            {/* ===== schedule appointment ===== */}
            {modalType === "scheduleAppointment" &&
              renderScheduleAppointmentModal()}
            ...
            {/* ===== edit appointment ===== */}
            {modalType === "editAppointment" && renderEditAppointmentModal()}
          </div>
        </div>
      </div>
    );
...
// safe accessor for ReceptionistContext
