...
export const patientAPI = {
  ...
  rescheduleAppointment: async (appointmentId, newSlotData) => {
    try {
      const response = await api.put(`/patient/appointments/${appointmentId}`, newSlotData);
      return { success: true, data: response.data };
    } catch (error) {
  ...
  getAvailableSlots: async () => {
    try {
      ...
      return { success: false, message: error.response?.data?.message || 'Failed to fetch available slots' };
    }
  },
  
  getDoctorBookedDates: async (doctorId) => {
    try {
      const response = await api.get(`/patient/doctors/${doctorId}/booked-dates`);
      return { success: true, data: response.data };
    } catch (error) {
      return { success: false, message: error.response?.data?.message || 'Failed to fetch booked dates' };
    }
  },

  getDoctorBookedTimes: async (doctorId, date) => {
    try {
      const response = await api.get(`/patient/doctors/${doctorId}/booked-times`, { params: { date } });
      return { success: true, data: response.data };
    } catch (error) {
      return { success: false, message: error.response?.data?.message || 'Failed to fetch booked times' };
    }
  },
...
export const receptionistAPI = {
  ...
  updatePatient: async (patientId, patientData) => {
    try {
      const response = await api.put(`/receptionist/patients/${patientId}`, patientData);
      return { success: true, data: response.data };
    } catch (error) {
  ...
  scheduleAppointment: async (appointmentData) => {
    try {
      const response = await api.post('/receptionist/appointments', appointmentData);
      return { success: true, data: response.data };
    } catch (error) {
      return { success: false, message: error.response?.data?.message || 'Failed to schedule appointment' };
    }
  },
  ...
  getTodayAppointments: async () => {
    try {
      ...
      return { success: false, message: error.response?.data?.message || 'Failed to fetch today\'s appointments' };
    }
  },
  
  checkInPatient: async (appointmentId) => {
    try {
      const response = await api.put(`/receptionist/appointments/${appointmentId}/checkin`);
      return { success: true, data: response.data };
    } catch (error) {
      return { success: false, message: error.response?.data?.message || 'Failed to check in patient' };
    }
  },
...
export const createPatientAppointment = (appointmentData) => patientAPI.bookAppointment(appointmentData);
export const updatePatientAppointment = (id, appointmentData) => patientAPI.rescheduleAppointment(id, appointmentData);
